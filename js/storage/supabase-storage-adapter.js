...
export class SupabaseStorageAdapter extends StoragePort {
  ...
  async upload(asset, file, options = {}) {
    ...
    if (error) throw new StorageError('UPLOAD_REJECTED', 'Unable to upload media.');
    return { ok: true };
  }
  ...
  async exists(asset) {
    ...
    if (error) throw new StorageError('READ_FORBIDDEN', 'Unable to inspect media.');
  ...
  async remove(asset) {
    ...
    if (error) throw new StorageError('DELETE_FORBIDDEN', 'Unable to delete media.');
    return { ok: true };
  }
}
