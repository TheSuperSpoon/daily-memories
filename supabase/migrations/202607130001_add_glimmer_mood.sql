...
begin
  if not exists (
    ...
    where conname = 'glimmers_mood_check'
      and conrelid = 'public.glimmers'::regclass
  ) then
    alter table public.glimmers
      add constraint glimmers_mood_check
      check (mood is null or mood in ('happy','neutral','sad','tired','loved'));
  end if;
end $$;

drop function if exists public.begin_glimmer_upload(uuid,date,text,bigint,text);
...
begin
  ...
  insert into public.glimmers(space_id,owner_id,role,glimmer_date,note,mood)
  values(p_space_id,auth.uid(),m.role,p_date,coalesce(p_note,''),p_mood) returning * into g;
...
begin
  ...
  return query select jsonb_build_object('id',g.id,'space_id',g.space_id,'owner_id',g.owner_id,
    'role',g.role,'glimmer_date',g.glimmer_date,'note',g.note,'mood',g.mood,'created_at',g.created_at,'ready_at',g.ready_at,
...
revoke all on function public.begin_glimmer_upload(uuid,date,text,bigint,text,text) from public, anon;
grant execute on function public.begin_glimmer_upload(uuid,date,text,bigint,text,text) to authenticated;
