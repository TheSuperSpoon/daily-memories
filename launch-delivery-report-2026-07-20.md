...
## Gift 私有音频（2026-07-21）

- `In Loving Memory.MP3` 已上传至私有 Supabase `gifts` bucket；前端不引用本地路径或公开 URL。
- Mel 点击第一个隐藏礼物后取得短期签名地址，并将完整音频写入同设备 Cache Storage；Gift 页面仍需 5/5 才可进入。
- 音频以 SHA-256 版本化缓存，礼物重置到 0/5 时自动清除本地缓存。
- 实测首次完整缓存 6.10 秒，后台缓存完成后首次进入播放器 171 ms，同设备缓存再次进入 247 ms。
- 实际播放推进至 1.24 秒，媒体时长 340.296 秒，浏览器 `readyState=4`；公开对象读取返回 400。
- 冷缓存演示后已删除临时 Mel：注册槽恢复 `open`，礼物为 0/5，本地缓存为 `empty`，旧会话与旧开场信完成标记均已清除。
- localhost 保持在 Mel 注册界面；下一位 Mel 注册后从生日微光信件开始。
- 单元测试 46/46 通过，远端 Supabase schema lint 无错误。
